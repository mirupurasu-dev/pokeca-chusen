...
- 📈 **期待値（相場−定価）**を自動算出して表示
- 🔗 **応募/販売リンク**を自動取得
- 🧩 **フォーム自動入力**：抽選応募フォームの氏名・住所・電話などをブラウザが自動入力（**送信は絶対にしない**・個人情報は端末内のみ）。設定手順は https://mirupurasu-dev.github.io/pokeca-chusen/autofill.html
...
- **相場**：`スニダン`（主）／`駿河屋`（副）の相場・定価。BOX名で検索してトップ結果を採用。名前が一致しない場合は「参考・要確認」と表示し、期待値は出しません。未発売商品は相場が無いため期待値なしになります。

### 網羅性について（正直な現状）

主要な通販・チェーン・カードショップ（Amazon・ポケセン・晴れる屋・ホビステ・シーガル・イエサブ・竜星のPAO・トレカプラザ・あみあみ・セブンネット・ノジマ・ゲオ・イオン各社・イトーヨーカドー・しまむら・ジョーシン等）は概ねカバーしていますが、**100%ではありません**。2ソースのまとめが載せた範囲に依存します。

現状で取りこぼしやすい主なチャネル（要検討の追加候補）：
- **家電量販店**：ビックカメラ / ヨドバシ / ヤマダデンキ / エディオン の抽選
- **一部専門店・地方店**：カードラボ / 遊ing（九州）など
- **ポケモンセンターオンライン公式の30周年抽選**（当選規模最大・近日開始）

これらを追加したい場合は `src/scrapers/` に専用ソースを足せます（`docs/adding-sources.md` 参照は未整備。要望ベースで追加）。
